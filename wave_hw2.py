...
def Q3(WAVE_NOW_FILENAME):
    ...
    def tuneChannel(data , chunksize):
	ret = []
    ...
    channels_data = [wave_data[i::params[0]] for i in xrange(params[0])]
    chunksize = params[2] / 8
    for i in range(len(channels_data)):
	channels_data[i] = tuneChannel(channels_data[i][0] , chunksize)
    datatype =  np.dtype('<i' + str(params[1]))
    #print datatype
    channels_data = np.array(channels_data , dtype = datatype)
    wave_write(channels_data , "Q3.wav" , params)
...
def Q4(WAVE_NOW_FILENAME):
    wave_data, time, params = wave_open(WAVE_NOW_FILENAME)
    echo = wave_data
    for i in range(params[3] - 10000):
	    wave_data[0][i + 10000] += echo[0][i] * 0.6
	    wave_data[1][i + 10000] += echo[1][i] * 0.6
